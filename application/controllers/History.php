...
class History extends CI_Controller
{
  ...
  public function getDataByUser()
  {
    ...
    $rows = $this->db->query("
      ...
      B.harga,
      B.subtotal,
      CASE WHEN A.status_penjualan = 'DITERIMA' THEN B.status ELSE A.status_penjualan END status_penjualan
      ...
      -- LEFT JOIN tb_penilaian D ON D.id_penjualan = A.id_penjualan AND D.id_barang = B.id_barang
  ...
  public function penilaian()
  {
    ...
    $this->db->query("UPDATE tb_dtl_penjualan SET status='SELESAI' 
    WHERE id_penjualan='" . $this->input->post('id_penjualan') . "' 
    AND id_barang='" . $this->input->post('id_barang') . "'");

    $SISA_REVIEW = $this->db->query("SELECT 
    (COUNT(*) - SUM(CASE WHEN `status`='SELESAI' THEN 1 ELSE 0 END)) AS SISA_REVIEW
    FROM tb_dtl_penjualan 
    WHERE id_penjualan='" . $this->input->post('id_penjualan') . "'")->row()->SISA_REVIEW;

    if ($SISA_REVIEW < 1) {
      $this->db->query("UPDATE tb_penjualan SET status='SELESAI' 
      WHERE id_penjualan='" . $this->input->post('id_penjualan') . "'");
    }
